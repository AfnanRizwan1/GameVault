...
export default function AdminUsers() {
  const [users, setUsers] = useState([...mockUsers, { id: 4, name: 'Jordan Kim', email: 'jordan@example.com', role: 'customer', joinDate: '2024-02-10', studio: null }, { id: 5, name: 'Riley Stone', email: 'riley@example.com', role: 'developer', joinDate: '2023-12-01', studio: 'PixelForge' }, { id: 6, name: 'Casey Lee', email: 'casey@example.com', role: 'customer', joinDate: '2024-04-15', studio: null }]);
  ...
  const filtered = users.filter(u => { const q = search.toLowerCase(); return (!q || u.name.toLowerCase().includes(q) || u.email.toLowerCase().includes(q)) && (!roleFilter || u.role === roleFilter); });
  const handleDelete = (id) => { setUsers(prev => prev.filter(u => u.id !== id)); setDeleteId(null); };
  const handleEditSave = (e) => { e.preventDefault(); setUsers(prev => prev.map(u => u.id === editUser.id ? editUser : u)); setEditUser(null); };
  const handleAddUser = (e) => { e.preventDefault(); setUsers(prev => [{ id: Date.now(), ...newUser, joinDate: new Date().toISOString().split('T')[0] }, ...prev]); setNewUser({ name: '', email: '', role: 'customer' }); setShowAddModal(false); };
