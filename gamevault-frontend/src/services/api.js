...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new ApiError('Unable to reach GameVault API. Is the backend running?', 0, null);
  }
...
export function normalizeGame(game) {
  ...
  const genre = game.genre || game.category || 'Action';
  ...
  return {
    ...
    coverImage: image,
    screenshots,
    developer: game.developer || game.developerName || 'Unknown Studio',
...
export function normalizeUser(user) {
  ...
  const name = user.name || user.profile?.fullName || user.username || 'GameVault User';
...
export function normalizeReview(review) {
  ...
  return {
    ...
    userName: review.userName || 'GameVault User',
...
export const gamesApi = {
  ...
  download: async (id, filename = 'gamevault-download') => {
