...
import { FiArrowRight, FiShoppingCart, FiStar, FiTrendingUp, FiZap, FiCheck } from 'react-icons/fi';
      ...
      <div className="hero-slide" style={{ background: `linear-gradient(135deg, ${colors[0]}22, ${colors[1]}11, var(--bg))` }}>
        ...
        <div className="hero-content">
          <div className="hero-left">
            {game.isNew && <div className="hero-badge-new"><FiZap /> NEW RELEASE</div>}
            ...
            <div className="hero-actions">
              ...
              <div className="hero-btns">
                {owned ? (
                  <Link to="/library" className="btn btn-success btn-lg"><FiCheck /> In Library</Link>
                ) : inCart ? (
                  <Link to="/cart" className="btn btn-primary btn-lg"><FiShoppingCart /> View Cart</Link>
                ) : (
                  <button className="btn btn-primary btn-lg" onClick={() => {
                    if (!user) { navigate('/login'); return; }
                    addToCart(game);
                  }}>
                    <FiShoppingCart /> {game.price === 0 ? 'Add to Library' : 'Add to Cart'}
                  </button>
                )}
...
function SectionHeader({ title, icon: Icon, linkTo, linkText }) {
  return (
    <div className="section-header">
      <h2 className="section-title"><Icon /> {title}</h2>
      {linkTo && <Link to={linkTo} className="section-more">{linkText} <FiArrowRight /></Link>}
    </div>
  );
}
    ...
    <div className="home-page">
      ...
      <div className="home-sections page-content">
        ...
        <section className="mb-6">
          <SectionHeader title="New Releases" icon={FiZap} linkTo="/store?filter=new" linkText="See All" />
        ...
        {/* Top Sellers */}
        <section className="mb-6">
          <SectionHeader title="Top Sellers" icon={FiTrendingUp} linkTo="/store?filter=top" linkText="See All" />
          <div className="games-grid">
        ...
        {/* Free to Play */}
        <section className="mb-6">
          <SectionHeader title="Free to Play" icon={FiStar} linkTo="/store?filter=free" linkText="See All" />
          <div className="games-grid">
        ...
        {/* Category Grid */}
        <section>
          <SectionHeader title="Browse by Genre" icon={FiStar} />
